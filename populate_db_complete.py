...
from parco_verismo.models import (
    Autore,
    Opera,
    Evento,
    Notizia,
...
def create_superuser():
    ...
    if created:
        ...
        user.save()
        print(f"✓ Superuser creato: {username}")
    else:
        ...
        user.save()
        print(f"• Superuser aggiornato: {username}")
...
def copy_static_to_media(source_path, destination_relative):
    ...
    static_source = os.path.join(
        settings.BASE_DIR, "parco_verismo", "static", "assets", "img", source_path
    )
...
def populate():
    ...
    verga, created = Autore.objects.get_or_create(
        slug="giovanni-verga", defaults={"nome": "Giovanni Verga"}
    )
    if created:
    ...
    capuana, created = Autore.objects.get_or_create(
        slug="luigi-capuana", defaults={"nome": "Luigi Capuana"}
    )
    if created:
...
l'eterno sfruttamento delle masse.""",
    ...
    for opera_data in opere_verga:
        ...
        if "copertina_path" in opera_data and not opera.copertina:
            copertina_path = copy_static_to_media(
                opera_data["copertina_path"],
                f"copertine_opere/{opera_data['slug']}.jpg",
            )
            if copertina_path:
                media_path = os.path.join(settings.MEDIA_ROOT, copertina_path)
                with open(media_path, "rb") as f:
                    opera.copertina.save(
                        f"{opera_data['slug']}.jpg", File(f), save=False
                    )
...
La visita è gratuita e dura circa 3 ore. Richiesta obbligatoria.""",
    ...
    for evento_data in eventi_data:
        ...
        if "immagine_path" in evento_data and not evento.immagine:
            image_path = copy_static_to_media(
                evento_data["immagine_path"], f"eventi/{evento_data['slug']}.jpg"
            )
            if image_path:
                media_path = os.path.join(settings.MEDIA_ROOT, image_path)
                with open(media_path, "rb") as f:
                    evento.immagine.save(
                        f"{evento_data['slug']}.jpg", File(f), save=False
                    )
...
raccontate dai grandi autori del verismo.""",
    ...
    for notizia_data in notizie_data:
        notizia, created = Notizia.objects.get_or_create(
            ...
            defaults={"is_active": notizia_data.get("is_active", True)},
        )
        if not created:
        ...
        if "immagine_path" in notizia_data and not notizia.immagine:
            image_path = copy_static_to_media(
                notizia_data["immagine_path"], f"notizie/{notizia_data['slug']}.jpg"
            )
            if image_path:
                media_path = os.path.join(settings.MEDIA_ROOT, image_path)
                with open(media_path, "rb") as f:
                    notizia.immagine.save(
                        f"{notizia_data['slug']}.jpg", File(f), save=False
                    )
    ...
    foto_vizzini = [
        (
            "vizzini/casaVerga.jpg",
            "Casa di Giovanni Verga",
            "La casa natale di Giovanni Verga a Vizzini, oggi museo dedicato allo scrittore.",
            "Luoghi",
        ),
        (
            "vizzini/centrostorico.jpg",
            "Centro Storico di Vizzini",
            "Il suggestivo centro storico di Vizzini con le sue caratteristiche vie.",
            "Luoghi",
        ),
        (
            "vizzini/duomo.jpg",
            "Duomo di Vizzini",
            "Il Duomo di Vizzini, uno dei principali monumenti storici della città.",
            "Luoghi",
        ),
        (
            "vizzini/borgo.jpg",
            "Borgo di Vizzini",
            "Il caratteristico borgo di Vizzini che ha ispirato molte opere di Verga.",
            "Luoghi",
        ),
        (
            "vizzini/cunziria.jpg",
            "La Cunziria",
            "L'antica conceria di Vizzini, luogo caratteristico del paese.",
            "Luoghi",
        ),
        (
            "vizzini/festa.jpeg",
            "Festa a Vizzini",
            "Una festa tradizionale a Vizzini durante le celebrazioni estive.",
            "Eventi",
        ),
        (
            "vizzini/festaRicotta.jpg",
            "Festa della Ricotta",
            "La tradizionale festa della ricotta di Vizzini.",
            "Eventi",
        ),
        (
            "vizzini/processione.jpg",
            "Processione religiosa",
            "Una processione religiosa per le vie di Vizzini.",
            "Eventi",
        ),
        (
            "vizzini/verga.jpeg",
            "Giovanni Verga",
            "Ritratto di Giovanni Verga, il grande scrittore verista.",
            "Personaggi",
        ),
        (
            "vizzini/bosco.jpeg",
            "Bosco di Vizzini",
            "Il paesaggio naturale intorno a Vizzini.",
            "Luoghi",
        ),
    ]
    ...
    foto_mineo = [
        (
            "mineo/Casa-Luigi-Capuana.jpg",
            "Casa di Luigi Capuana",
            "La casa natale di Luigi Capuana a Mineo.",
            "Luoghi",
        ),
        (
            "mineo/centro-storico.jpg",
            "Centro Storico di Mineo",
            "Il centro storico di Mineo con i suoi vicoli caratteristici.",
            "Luoghi",
        ),
        (
            "mineo/castello.jpg",
            "Castello di Mineo",
            "L'antico castello di Mineo, simbolo della città.",
            "Luoghi",
        ),
        (
            "mineo/panorama.jpg",
            "Panorama di Mineo",
            "Il suggestivo panorama dai colli di Mineo.",
            "Luoghi",
        ),
        (
            "mineo/chiesa.jpg",
            "Chiesa di Mineo",
            "Una delle chiese storiche di Mineo.",
            "Luoghi",
        ),
        (
            "mineo/premio-luigi.jpg",
            "Premio Luigi Capuana",
            "La cerimonia del Premio letterario intitolato a Luigi Capuana.",
            "Eventi",
        ),
        (
            "mineo/santa-agrippina.jpg",
            "Chiesa di Santa Agrippina",
            "La chiesa dedicata a Santa Agrippina, patrona di Mineo.",
            "Luoghi",
        ),
    ]
    ...
    foto_licodia = [
        (
            "licodia/hero-bg.jpg",
            "Licodia Eubea",
            "Veduta panoramica di Licodia Eubea.",
            "Luoghi",
        ),
        (
            "licodia/chiesa_santa_margherita.jpg",
            "Chiesa di Santa Margherita",
            "La chiesa di Santa Margherita a Licodia Eubea.",
            "Luoghi",
        ),
        (
            "licodia/colle_castello.jpg",
            "Colle del Castello",
            "Il colle del castello di Licodia Eubea.",
            "Luoghi",
        ),
        (
            "licodia/panorama.jpg",
            "Panorama di Licodia",
            "Il suggestivo panorama dai colli di Licodia Eubea.",
            "Luoghi",
        ),
        (
            "licodia/settimana_santa.jpg",
            "Settimana Santa",
            "Le celebrazioni della Settimana Santa a Licodia Eubea.",
            "Eventi",
        ),
        (
            "licodia/festa_dell_uva.jpeg",
            "Festa dell'Uva",
            "La tradizionale festa dell'uva a Licodia Eubea.",
            "Eventi",
        ),
        (
            "licodia/sagra.jpg",
            "Sagra locale",
            "Una sagra tradizionale a Licodia Eubea.",
            "Eventi",
        ),
    ]
    ...
    for comune, foto_list in tutte_le_foto:
        for immagine_path, titolo, descrizione, categoria in foto_list:
            ...
            if not foto.immagine:
                # Copia l'immagine
                image_path_rel = copy_static_to_media(
                    immagine_path,
                    f"archivio_fotografico/{comune.lower()}_{foto_slug}.jpg",
                )
                if image_path_rel:
...
della Settimana Santa e i paesaggi che ispirarono diversi autori veristi.""",
    ...
    for itinerario_data in itinerari_data:
        ...
        if (
            "immagine_path" in itinerario_data
            and itinerario_data["immagine_path"]
            and not itinerario.immagine
        ):
            image_path = copy_static_to_media(
            ...
            if image_path:
                try:
                    media_path = os.path.join(settings.MEDIA_ROOT, image_path)
                    with open(media_path, "rb") as f:
                        itinerario.immagine.save(
                            f"{itinerario_data['slug']}.jpg", File(f), save=False
                        )
                except Exception as e:
                    print(
                        f"  ⚠️  Errore salvataggio immagine itinerario {itinerario_data['slug']}: {e}"
                    )
    ...
    print(
        f"  - Itinerari verghiani: {Itinerario.objects.filter(tipo='verghiano').count()}"
    )
    print(
        f"  - Itinerari capuaniani: {Itinerario.objects.filter(tipo='capuaniano').count()}"
    )
    print(
        f"  - Itinerari tematici: {Itinerario.objects.filter(tipo='tematico').count()}"
    )
...
def check_database():
    ...
    itinerari_senza_coords = Itinerario.objects.filter(
        coordinate_tappe__isnull=True
    ).count()
