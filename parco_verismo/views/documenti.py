...
def archivio_fotografico_view(request):
    ...
    foto_verga = FotoArchivio.objects.filter(is_active=True, autore="VERGA").order_by(
        "ordine", "-data_aggiunta"
    )
    foto_capuana = FotoArchivio.objects.filter(
        is_active=True, autore="CAPUANA"
    ).order_by("ordine", "-data_aggiunta")
    foto_altro = (
        FotoArchivio.objects.filter(is_active=True)
        .exclude(autore__in=["VERGA", "CAPUANA"])
        .order_by("ordine", "-data_aggiunta")
    )
